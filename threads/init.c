...
/** The maximum possible command length */
#define MY_MAX_CMD_LENGTH 50
...
int my_deal_with_cmd(const uint8_t *str);
/** ----- Here is the end --------*/
...
/** parse the command and do the Corresponding task 
    return -1 if I should exit, return 0 otherwise 
    strlen(str) == length */
int my_deal_with_cmd(const uint8_t *str)
{
  if(strcmp((const char *)str,"whoami")==0)
  {
    putbuf("2000013058\n",11);
    return 0;
  }
  else if(strcmp((const char *)str,"exit")==0)
  {
    return -1;
  }
  else
  {
    putbuf("invalid command\n",16);
    return 0;
  }
}
...
{
#ifdef USERPROG
  my_page_initialized_flag = 1;
  my_tss_initialized_flag = 1;
  my_init_finish_flag = 0;
#endif
  char **argv;
...
#ifdef USERPROG
  my_page_initialized_flag = 0;
  my_tss_initialized_flag = 0;
#endif
  ...
  if (*argv != NULL) {
    ...
    putbuf("PKUOS>",6);
    uint8_t my_cmd[MY_MAX_CMD_LENGTH];
    uint8_t my_char;
    int my_idx=0;
    while(1){
      my_char=input_getc();
      if(my_char=='\r'||my_char=='\n'){
        if(my_char=='\r')putchar(my_char);
        putchar('\n');
        my_cmd[my_idx]='\0';
        if(my_deal_with_cmd(my_cmd) == -1){
          break;
        }
        putbuf("PKUOS>",6);
        my_idx=0;
      }
      else if(my_char==(uint8_t)'\b'||my_char==(uint8_t)127){
        if(my_idx == 0){
          continue;
        }
        my_idx--;
